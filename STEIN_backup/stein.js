...
//da qui,  miei esperimenti
var i = 0;
var array = [];
var logBackup = console.log;
var logMessages = [];
var system_question_number = 1;
var starting_node = 0;
var to_scroll = "NO";
var started_logging = "NO";
var left = "NO"
var top = "NO"
var one = "";
var two = "";
var prev_translated_x = 0;
var prev_translated_y = 0;
var prev_node = 0;
var check_start_path = 0;
var tmp_path = "NO";
var first_interaction = "YES_apri";
var warning = 0;
var brush_start_info;
var brush_end_info;
var pan_extent_x = 0;
var pan_extent_y = 0;
var check_pan = 0;
var check_handle = 0;
var what_handle = "";
var left_handle_pos = 0;
var right_handle_pos = 0;
var old_brush;
var pan_info;
var clicked_x, clicked_y;
var current_mousemove;
var old_event;
var check_mousemove_distance = 0;
var check_mousemove_departure = 0;
var check_mousemove_arrival = 0;
var check_mousemove_delay = 0;
var check_mousemove_airtime = 0;
var array_questionnaire = [];
var z = 0;
var brushed_departure = 0;
var brushed_distance = 0;
var brushed_arrival = 0;
var brushed_airtime = 0;
var brushed_delay = 0;
var warning_delay = 0;
var wheel_count = 0;
var started_brushing = 0;
var detected_wheel = 0;
var detected_mouseout = 0;
////////////

var start_departure_x = 0;
var end_departure_x = 0;
var start_distance_x = 0;
var end_distance_x = 0;
var start_arrival_x = 0;
var end_arrival_x = 0;
var start_airtime_x = 0;
var end_airtime_x = 0;
var start_delay_x = 0;
var end_delay_x = 0;
var pity_timer = 0;
...
(function (console) {
    ...
    console.save = function (data, filename) {
        ...
        var blob = new Blob([finalData], { type: 'text/json' }),
...
window.Stein = new class {
    ...
    parseEvent(e, sourceWindowName) {
        let _this = this;
        let lowLevelTypes = ["click", "dblclick", "wheel", "change", "mousedown", "mouseup", "shown", "mouseover", "brush"];
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        /*if((e.vegaType == "mousedown") & (e.target.parentNode.id == "distance"))*/
        if (e.vegaType != "mousemove") console.log(e);

        if (e.target.parentNode.id == "app") return;
        if (e.target.parentNode.id == "date") return;


        //5 is not present in the generated path
        if (e.target.parentNode.id == "count") xpath_helper = "1";
        else if (e.target.parentNode.id == "distance") xpath_helper = "2";
        else if (e.target.parentNode.id == "arrival") xpath_helper = "3";
        else if (e.target.parentNode.id == "departure") xpath_helper = "4";
        else if (e.target.parentNode.id == "airtime") xpath_helper = "6";
        else if (e.target.parentNode.id == "delay") xpath_helper = "7";
        var translated_x = e.clientX; //per la scrollbar
        var translated_y = e.clientY;
        var mio_evento;

        /* pageX and pageY
        Being based on the edge of the document as it is, this property takes into account
        any horizontal scrolling of the page. For example, if the page is scrolled such that 200 pixels
        of the left side of the document are scrolled out of view,
        and the mouse is clicked 100 pixels inward from the left edge of the view,
        the value returned by pageX will be 300.
        */

        if (e.vegaType == "mousemove") {


            if (e.target.parentNode.id == "count") current_mousemove = "count";
            else if (e.target.parentNode.id == "distance") current_mousemove = "distance";
            else if (e.target.parentNode.id == "arrival") current_mousemove = "arrival";
            else if (e.target.parentNode.id == "departure") current_mousemove = "departure";
            else if (e.target.parentNode.id == "airtime") current_mousemove = "airtime";
            else if (e.target.parentNode.id == "delay") current_mousemove = "delay";


            let info = {
                coord_x: e.offsetX,  //clientX e clientY prima
                coord_y: e.offsetY   //offset rispetto al visual component che tocco: vado ad eludere il problema delle scroll
            }



            mio_evento = {
                xpath: "/html[1]/body[1]/div[2]/div[" + xpath_helper + "]" + "/canvas[1]",
                css: "#" + e.target.parentNode.id + " " + "canvas.marks",
                startingPath: "-1", // o 2 nel caso speciale
                siblings: "0", //o 4 nel caso speciale
                event: e.vegaType,
                info,
                leadsToState: 0, // da 0 a 8...
            }

            if ((current_mousemove == "distance") & (check_mousemove_distance == 0)) {
                console.log("epic")
                array[i] = mio_evento;
                i++;
                check_mousemove_distance = 1;
            }

            if ((current_mousemove == "arrival") & (check_mousemove_arrival == 0)) {
                console.log("epic")
                array[i] = mio_evento;
                i++;
                check_mousemove_arrival = 1;
            }

            if ((current_mousemove == "departure") & (check_mousemove_departure == 0)) {
                console.log("epic")
                array[i] = mio_evento;
                i++;
                check_mousemove_departure = 1;
            }

            if ((current_mousemove == "airtime") & (check_mousemove_airtime == 0)) {
                console.log("epic")
                array[i] = mio_evento;
                i++;
                check_mousemove_airtime = 1;
            }

            if ((current_mousemove == "delay") & (check_mousemove_delay == 0)) {
                console.log("epic")
                array[i] = mio_evento;
                i++;
                check_mousemove_delay = 1;
            }
        ...
        if (e.vegaType == "click") {
            ...
            if ((e.offsetX >= 375) & (e.offsetX <= 439)) {
                if ((e.offsetY >= 7) & (e.offsetY <= 20)) {

                    if (e.target.parentNode.id == "departure") {
                        brushed_departure = 0;
                        start_departure_x = 0;
                        end_departure_x = 0;
                    }
                    else if (e.target.parentNode.id == "distance") {
                        brushed_distance = 0;
                        start_distance_x = 0;
                        end_distance_x = 0;
                    }
                    else if (e.target.parentNode.id == "arrival") {
                        brushed_arrival = 0;
                        start_arrival_x = 0;
                        end_arrival_x = 0;
                    }
                    else if (e.target.parentNode.id == "airtime") {
                        brushed_airtime = 0;
                        start_airtime_x = 0;
                        end_airtime_x = 0;
                    }
            ...
            else if ((e.offsetX >= 230) & (e.offsetX <= 300)) {

                if ((e.offsetY >= 5) & (e.offsetY <= 20)) {

                    if (e.target.parentNode.id == "delay") {
                        brushed_delay = 0;
                        start_delay_x = 0;
                        end_delay_x = 0;
                    }
        ...
        else if (e.vegaType == "wheel") {



            var tmp, tmp_letter = "M", tmp_x = e.offsetX, tmp_y = e.offsetY; //altre lettere possibili: L o B tiop

            if (e.wheelDelta < 0) tmp = "out";
            else tmp = "in";


            let info = {
                direction: tmp,
                letter: tmp_letter + "quadra_aperta", //serve per la traduzione del log
                x: tmp_x,
                y: tmp_y + "quadra_chiusa" //serve per la traduzione del log
            ...
            if ((wheel_count % 5 == 0) & (started_brushing != 1)) {
                array[i] = mio_evento;
                i++;
            }

            else if (started_brushing == 1) {
                detected_wheel = 1;
            }
            /*
        ...
        //se è mousedown, l'unica cosa che può fare dopo è una sequenza di mousemove e una mouseup finale. Trasformo tutto in una brush. Imposto warning a 1, che non mi cattura il resto.
        else if ((e.vegaType == "mousedown") & (warning != 1)) {
            ...
            array[i] = fictitious_mousemove;
            i++;


            if (e.target.parentNode.id != "delay") { //se è diverso da delay, che è l'unico brush bidimensionale, allora non mi interessa la y e la setto a 50
                brush_start_info = {
                    start_brush_x: e.offsetX,
                    start_brush_y: 50

                }
            }

            else if (e.target.parentNode.id == "delay") { //se è diverso da delay, che è l'unico brush bidimensionale, allora non mi interessa la y e la setto a 50
                warning_delay = 1;
                brush_start_info = {

                    start_brush_x: e.offsetX,
                    start_brush_y: e.offsetY
            ...
            mio_evento = {
                //doesPathStart: first_interaction,
                xpath: "/html[1]/body[1]/div[2]/div[" + xpath_helper + "]" + "/canvas[1]",
                css: "#" + e.target.parentNode.id + " " + "canvas.marks",
                startingPath: "-1", // o 2 nel caso speciale
                siblings: "0", //o 4 nel caso speciale
                event: "brush",
                pan_info,
                info: brush_start_info
            }
        }


        ///se io ho una mousedown compresa tra l'area che devo brushare, allora devo catturare da lì fino a mouseup, capire la distanza e il gioco è fatto.
        else if (e.vegaType == "mouseup") {

            started_brushing = 0;

            brush_end_info = {
                end_brush_x: e.offsetX,
                end_brush_y: 50

            }

            right_handle_pos = brush_end_info.end_brush_x;

            if (brush_end_info.end_brush_x < brush_start_info.start_brush_x) { //il furbacchione ha fatto il brush al contrario ...
                let tmp = brush_end_info.end_brush_x;
                brush_end_info.end_brush_x = brush_start_info.start_brush_x;
                brush_start_info.start_brush_x = tmp;
            }

            if ((brush_end_info.end_brush_y < 0) || (brush_end_info.end_brush_y > 250)) { //il furbacchione ha portato il mouse troppo lontano...
                brush_end_info.end_brush_y = 50;
            }

            if ((brush_start_info.start_brush_y < 0) || (brush_start_info.start_brush_y > 250)) {
                brush_start_info.start_brush_y = 50;
            }
            ...
            mio_evento = {
                ...
                event: "brush",
                pan_info,
                info: JSON.stringify(brush_start_info) + JSON.stringify(brush_end_info) + "chiudi_brush"
            }

            if (detected_mouseout == 1) mio_evento.startingPath = "detected_mouseout";
            if (detected_wheel == 1) mio_evento.startingPath = "detected_wheel";

            if (brush_end_info.end_brush_x - brush_start_info.start_brush_x >= 3) {
                old_brush = e.target.parentNode.id;
                array[i] = mio_evento;
                i++;
                warning = 1;
                check_handle = 1;
                ...
                if (e.target.parentNode.id == "departure") {
                    start_departure_x = brush_start_info.start_brush_x;
                    end_departure_x = brush_end_info.end_brush_x;
                    brushed_departure = 1;
                }
                else if (e.target.parentNode.id == "distance") {
                    start_distance_x = brush_start_info.start_brush_x;
                    end_distance_x = brush_end_info.end_brush_x;
                    brushed_distance = 1;

                }
                else if (e.target.parentNode.id == "arrival") {
                    start_arrival_x = brush_start_info.start_brush_x;
                    end_arrival_x = brush_end_info.end_brush_x;
                    brushed_arrival = 1;

                }
                else if (e.target.parentNode.id == "airtime") {
                    start_airtime_x = brush_start_info.start_brush_x;
                    end_airtime_x = brush_end_info.end_brush_x;
                    brushed_airtime = 1;

                }
                else if (e.target.parentNode.id == "delay") {
                    start_delay_x = brush_start_info.start_brush_x;
                    end_delay_x = brush_end_info.end_brush_x;
                    brushed_delay = 1;
        ...
        if ((e.vegaType == "click") & (e.target.parentNode.id == "delay")) {
            console.log("ok1");

            console.log("ok2");
            brush_end_info = {
                end_brush_x: e.offsetX,
                end_brush_y: e.offsetY
            }
            ...
            if (brush_end_info.end_brush_y < brush_start_info.start_brush_y) { //il furbacchione ha fatto il brush al contrario anche sulle y ...
                let tmp = brush_end_info.end_brush_y;
                brush_end_info.end_brush_y = brush_start_info.start_brush_y;
                brush_start_info.start_brush_y = tmp;
            }
            ...
            */
            ...
            if ((brush_end_info.end_brush_x - brush_start_info.start_brush_x >= 3) & (warning_delay == 1)) {
                array[i] = mio_evento;
                old_brush = e.target.parentNode.id;
                i++;
                warning = 1;
        ...
        if (e.vegaType == "mousedown") {


            started_brushing = 1;

            //console.log(e.offsetX, e.offsetY, tmp_s_x, tmp_e_x, tmp_s_y, tmp_e_y);
            //if((e.offsetX >= tmp_s_x) & (e.offsetX <= tmp_e_x)){
            if ((e.offsetY <= 250) || (e.offsetY >= -150)) {//se sono all'interno di un vecchio brush e non sforo con le y
                console.log("pronto per il pan");
                //controllo se la mousedown è effettuata all'interno di un vecchio brush, così la prossima mouseup che viene catturata
                if (e.target.parentNode.id == "departure") {
                    if (brushed_departure == 1) {
                        if ((e.offsetX >= start_departure_x) & (e.offsetX <= end_departure_x)) {


                            pan_extent_x = e.offsetX;
                            pan_extent_y = e.offsetY;
                            clicked_x = e.offsetX;
                            clicked_y = e.offsetY;
                            check_pan = 1;
                            start_departure_x = start_departure_x + e.offsetX;
                ...
                if (e.target.parentNode.id == "distance") {
                    if (brushed_distance == 1) {
                        if ((e.offsetX >= start_distance_x) & (e.offsetX <= end_distance_x)) {
                            ...
                            clicked_y = e.offsetY;
                            check_pan = 1;
                            start_distance_x = start_distance_x + e.offsetX;
                ...
                if (e.target.parentNode.id == "arrival") {
                    if (brushed_arrival == 1) {
                        if ((e.offsetX >= start_arrival_x) & (e.offsetX <= end_arrival_x)) {
                            ...
                            clicked_y = e.offsetY;
                            check_pan = 1;
                            start_arrival_x = start_arrival_x + e.offsetX;
                ...
                if (e.target.parentNode.id == "airtime") {
                    if (brushed_airtime == 1) {
                        if ((e.offsetX >= start_airtime_x) & (e.offsetX <= end_airtime_x)) {
                            ...
                            clicked_y = e.offsetY;
                            check_pan = 1;
                            start_airtime_x = start_airtime_x + e.offsetX;
                        }
                ...
                if (e.target.parentNode.id == "delay") {
                    if (brushed_delay == 1) {
                        if ((e.offsetX >= start_delay_x) & (e.offsetX <= end_delay_x)) {
                            console.log("controllo delay")
                            pan_extent_x = e.offsetX;
                            pan_extent_y = e.offsetY;
                            clicked_x = e.offsetX;
                            clicked_y = e.offsetY;
                            check_pan = 1;
                            start_delay_x = start_delay_x + e.offsetX;
        ...
        if ((e.vegaType == "mouseup") & (check_pan == 1)) {



            pan_extent_x = e.offsetX - pan_extent_x;
            pan_extent_y = 0; //!!!!///
            console.log(pan_extent_x);
            ...
            if (e.target.parentNode.id == "departure") end_departure_x = end_departure_x + e.offsetX;
            else if (e.target.parentNode.id == "delay") end_delay_x = end_delay_x + e.offsetX;
            else if (e.target.parentNode.id == "distance") end_distance_x = end_distance_x + e.offsetX;
            else if (e.target.parentNode.id == "airtime") end_airtime_x = end_airtime_x + e.offsetX;
            else if (e.target.parentNode.id == "arrival") end_arrival_x = end_arrival_x + e.offsetX;


            if ((pan_extent_x != 0)) { //brush inutile & (pan_extent_y != 0)

                if (array[i - 2].event == "brush") {
                    array[i - 2].pan_info = info_p;
                }
        ...
        if ((e.vegaType == "click") & (check_pan == 1)) {

            if (e.target.parentNode.id == "delay") {

                console.log("ci siamo delay")
                pan_extent_x = e.offsetX - pan_extent_x;
                pan_extent_y = e.offsetY - pan_extent_y;
                ...
                left_handle_pos = e.offsetX + pan_extent_x;
                right_handle_pos = e.offsetX + pan_extent_x;

                if ((pan_extent_x != 0) & (pan_extent_y != 0)) {
        ...
        old_event = mio_evento;



        //vecchio evento per il detect della mousemove

        ////CHECK PER LEADSTOSTATE
        //MANCA EVENTO STRANO mousedown on distance che porta a stato 2

        function check_state(e, mio_evento) {
            if (starting_node == 0) {
                if ((e.vegaType == "mousedown") & (xpath_helper == 1)) {
                    mio_evento.leadsToState = 1;
                    starting_node = 1;
                }
            }

            if (starting_node == 0) {
                if ((e.vegaType == "mousedown") & (xpath_helper == 2)) {
                    mio_evento.leadsToState = 4;
                    starting_node = 4;
                }
            }
            ...
            if (starting_node == 0) {
                if ((e.vegaType == "mousedown") & (xpath_helper == 3)) {
                    mio_evento.leadsToState = 5;
                    starting_node = 5;
                }
            }

            if (starting_node == 0) {
                if ((e.vegaType == "mousedown") & (xpath_helper == 4)) {
                    mio_evento.leadsToState = 6;
                    starting_node = 6;
                }
            }

            if (starting_node == 0) {
                if ((e.vegaType == "mousedown") & (xpath_helper == 6)) {
                    mio_evento.leadsToState = 7;
                    starting_node = 7;
                }
            }

            if (starting_node == 0) {
                if ((e.vegaType == "mousedown") & (xpath_helper == 7)) {
                    mio_evento.leadsToState = 8;
                    starting_node = 8;
                }
            }
            ...
            if (starting_node == 1) {
                if ((e.vegaType == "mousemove") & (xpath_helper == 1)) {
                    mio_evento.leadsToState = 1;
                    starting_node = 1;
                }
            }
            ...
            if (starting_node == 1) {
                if ((e.vegaType == "mouseout") & (xpath_helper == 1)) {
                    mio_evento.leadsToState = 1;
                    starting_node = 1;
                }
            }
            ...
            if (starting_node == 1) {
                if ((e.vegaType == "click") & (xpath_helper == 1)) {
                    mio_evento.leadsToState = 1;
                    starting_node = 1;
                }
            }
            ...
            if (starting_node == 2) {
                if ((e.vegaType != "mousedown") & (xpath_helper == 2)) {
                    mio_evento.leadsToState = 2;
                    starting_node = 2;
                }
                else {
                    mio_evento.leadsToState = 3;
                    starting_node = 3;
                }
            ...
            if (starting_node == 3) {
                if ((e.vegaType != "mouseup") & (xpath_helper == 2)) {
                    mio_evento.leadsToState = 3;
                    starting_node = 3;
                }
                else {
            ...
            if (starting_node == 5) {
                if ((e.vegaType != "mouseup") & (xpath_helper == 3)) {
                    mio_evento.leadsToState = 5;
                    starting_node = 5;
                }
                else {
            ...
            ///////6//////
            if (starting_node == 6) {
                if ((e.vegaType != "mouseup") & (xpath_helper == 4)) {
                    mio_evento.leadsToState = 6;
                    starting_node = 6;
                }
                else {
            ...
            if (starting_node == 7) {
                if ((e.vegaType != "mouseup") & (xpath_helper == 6)) {
                    mio_evento.leadsToState = 7;
                    starting_node = 7;
                }
                else {
            ...
            if (starting_node == 8) {
                if ((e.vegaType != "mouseup") & (xpath_helper == 7)) {
                    mio_evento.leadsToState = 8;
                    starting_node = 8;
                }
                else {
        ...
        if ((mio_evento.css != "#app canvas.marks") & (mio_evento.event != null)) {
            if (warning != 1) {
                if ((mio_evento.event != "mousemove") & (mio_evento.event != "mouseout")) {
                    if ((mio_evento.event != "mouseup") & (mio_evento.event != "wheel")) {
                        if (mio_evento.event != "mousedown") {
                            array[i] = mio_evento;
                            array[i + 1] = "double!?!"
                            i++;
                        }
        ...
        //events_try = events_try + "//////" + e;
        //console.log(JSON.stringify(events_try))
        ...
        if (!isLowLewel && !isHighLevel && !isOverLevel) {
            return null;
        }
        ...
        /*
        if(isHighLevel) _this.highLevelEventTimestamp[sourceWindowName] = timestamp;
        else if(timestamp - _this.highLevelEventTimestamp[sourceWindowName] < 150) return null;
        */
        ...
        */
        ...
        _this.events[sourceWindowName].push(myEvent); /*prima era myevent e non "e"*/
        ...
        */
        return myEvent; //qua era return "myevent e non "e" "
    ...
    getPublicIp(callback) {
        $.getJSON('https://api.ipify.org?format=json', function (data) {
            if (data.hasOwnProperty("ip")) callback(data.ip)
        });
    }
...
; window.Stein.design = new class {
    ...
    start() {
        ...
        $.get('js/stein-events.txt', function (script) {
                                ...
                                _this.stein.watchEvents(iframe.contentWindow, function (event) {
                                    ...
                                    for (let i = 0; i < _this.data.events.length; i++) {
                                        ...
                                        for (let j = 0; j < _this.data.dataTypes.length; j++) {
                                            if (_this.data.dataTypes[j].name == tracedEvent.dataType) {
                                                datatypeToStringFunction = JSON.parse(JSON.stringify(_this.data.dataTypes[j])).toStringFunction;
                                                console.log("boh" + datatypeToStringFunction);
                                                break;
                                            }
                                        }
        ...
        _setPageContent(page, options) {
            ...
            */
            if (page == dom.pages.systemQuestions) {
                ...
                let question = _this.data.systemQuestions[_this.data.currentSystemQuestionIndex];
                ...
                bottomBar.append("button").attr("class", "btn btn-light")
                ...
                bottomBar.append("button").attr("class", "btn btn-primary")
                    ...
                    .on("click", function () {
                        ...
                        array[i] = "Prossima system question";
                        i++;
                        pity_timer++;
                        brushed_delay = 0;
                        brushed_departure = 0;
                        ...
                        /*if(pity_timer == 5){
                            ...
                            for(let w = 0; w<array.length; w++){
                                    ...
                                    if((array[w].event!="brush") & (array[w].pan_info != null)) array[w] = "deleted!";
                                    //if((array[w].event=="brush") & (array[w].css = "#delay canvas.marks")) array[w+4] = "deleted duplicate brush on delay";
                    ...
                    d3.select("body").on("SteinLibEventSystemLoaded", function () {
                        ...
                        _this.stein.watchEvents(iframe.contentWindow, function (event) {
                            ...
                            for (let i = 0; i < _this.data.config.events.length; i++) {
                                let tracedEvent = _this.data.config.events[i]; //////////////ARRAY DI EVENTI!!!/////////////

                                let fSpecialize = tracedEvent.specializeEvent + ".call(this, " + JSON.stringify(e) + ");"; //errore qui
            ...
            */
            if (page == dom.pages.surveyCompleted) {
                _this._generateSurveyResult();
                localStorage.setItem("evaluation", JSON.stringify(array))
                d3.select(_this.dom.saveButton).on("click", function () {
                    ...
                    */
                    ...
                    _this._showLoadingPage();
                    _this.stein.uploadJson(serverUploadUrl, "results/" + filename, "nope", function (done, error) {
                        if (done) {
                            _this.data.saved = true;
                            _this._saveSessionData();
                            _this._showPage(dom.pages.surveySaved);
                            ...
                            //_this.stein.downloadJson("survey_answers.json", _this._generateSurveyResult());

                            console.save(array, "evaluation_result.txt");
...
class SteinInitialQuestion extends SteinQuestion {
    ...
    toOutput() {
        ...
        array[i] = "INITIAL QUESTIONS";
        i++;
        array[i] = _this.id;
        i++;
        array[i] = _this.responseType;
        i++;
        array[i] = _this.response;
        i++;
        var r = {
            id: _this.id,
        ...
        if (event.dataType == _this.dataType && contributeToAnswer) {
            ...
            if (_this.responseType == SteinSystemQuestion.ResponseType.UserInteractionList) {
                ...
                data.forEach(function (d) {
                    var foundIndex = -1;
                    for (var j = 0; j < _this.response.length; j++) {
                        let s = eval(dataTypeToString + ".call(this, " + JSON.stringify(d) + ")");
                        if (s == _this.response[j]) {
                            foundIndex = j;
                            break;
                        }
                    }
