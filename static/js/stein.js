...
window.Stein = new class {
    ...
    parseEvent(e) {
        ...
        var evento = ""
        if(e.vegaType == null) evento = e.type
        else evento = e.vegaType

        var cssString = ""
        if(e.target.parentNode.id == "") css = e.fromElement.id
        else cssString = e.target.parentNode.id

        if(cssString == "") cssString = e.srcElement.__data__.id

        mio_evento = {
            xpath: getPathTo(e.target),
            css: "#" + cssString + " " + "canvas.marks",
            startingPath: "-1",
            siblings: "0",
            event: evento,
            info,
...
function getXPath(element) {
    ...
    while (element && element.nodeType === Node.ELEMENT_NODE) {
        ...
        if (parent.tagName)
            xpath = '/' + parent.tagName.toLowerCase() + xpath;
